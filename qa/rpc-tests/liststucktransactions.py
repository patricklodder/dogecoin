...
class ListStuckTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # the bad wallet tx
        stuck_excl_watchonly = wallet.liststucktransactions(0, False)
        ...
        stuck_incl_watchonly = wallet.liststucktransactions(0, True)
        ...
        # make sure the original bad tx gets fully serialized with verbosity=1
        stuck_incl_watchonly_hex = wallet.liststucktransactions(1, True)
        assert(bad_txhex in stuck_incl_watchonly_hex)
        assert(good_txhex not in stuck_incl_watchonly_hex)
